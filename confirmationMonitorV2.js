            ...
            } else
            {
                ...
                if (!confirmations || confirmations.length == 0) {
                    ...
                    for (const conf of confirmations) {
                        ...
                        //adding first confirmation data
                        // console.log(`check-- ${conf.txid} -- ${conf.blockheight} -- ${conf.blocktime}`)
                        tx.blockHeight = conf.blockheight;
                        tx.firstConfirmationTime = Math.round(conf.blocktime - (tx.timestamp/1000));
                        console.log(`First confirmation for ${tx.txHash} at block ${tx.blockHeight} and time ${tx.firstConfirmationTime}`);

                        // fetching sixth confirmation data 
                        const blockDetails = await fetchBlockDetails((tx.blockHeight + 5));
                        if(blockDetails){
                            tx.sixthConfirmationTime = Math.round(blockDetails.time - (tx.timestamp/1000));
                            console.log(`Sixth confirmation for ${tx.txHash} at block ${blockDetails.height} and time ${tx.sixthConfirmationTime}`);
                        }
                        tx.status = "completed"
                        modified = true;
                    }
