...
import {signTransaction_v1, base64ToHexCode } from './controllers/cosignerController.js';
// import { base64ToHex } from './utils/base64Utils.js';
import { checkMempool, decodedTransaction } from './controllers/wocController.js';
...
export default function () {
    ...
    const signedRawTx = signTransaction_v1(consignerHost, rawtx);
    console.log(`Signed Raw Transaction: ${signedRawTx}`);
    if (!signedRawTx) return;

    // Step 3: Get transaction hash
    const decodedHex = base64ToHexCode(signedRawTx);
    console.log(`decoded Hex: ${decodedHex}`);
    if (!decodedHex) return;
    
    const txHash = decodedTransaction(wocHost, decodedHex);
